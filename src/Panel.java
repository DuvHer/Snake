...
import java.awt.Color;  //libreria para  el  color
...
public class Panel extends JPanel implements KeyListener{
    ...
    ListaSimple lista = new ListaSimple(); // se crea una  lista  simple
    int puntos = 0;  //
    String marcha = "derecha";

    Panel() {
      
        label.setBounds(100, 250, 200, 50);  //es  el que  dice puntos 
        this.add(label);
        mov.start();
        this.setLayout(null);
        t.requestFocus();
    ...
    void crecer() {
        ...
        if (puntos == 20) {  //cuando llegue   a  20  se va aterminar  
    ...
    void validar() {
        if (btn[CabezaX][CabezaY].getBackground() == Color.GREEN) {
        } else if (btn[CabezaX][CabezaY].getBackground() == Color.BLUE) {
